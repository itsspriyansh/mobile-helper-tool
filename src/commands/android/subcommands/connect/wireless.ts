...
export async function connectWirelessAdb(sdkRoot: string, platform: Platform): Promise<boolean> {
  try {
    ...
    // Run the connect command using ip address and port number provided.
    // If the device is previously paired, then connection will succeed.
    // If not, then prompt user for pairing details and then proceed to connect.
